        ...
        <div className="p-8 md:p-10 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/30">
          ...
          <div className="mt-8 flex flex-col md:flex-row gap-4 justify-between items-center">
            <div className="flex flex-wrap gap-2">
              <FilterBtn label="All" active={filter === 'all'} onClick={() => setFilter('all')} color="bg-slate-100 dark:bg-slate-800" />
              <FilterBtn label="Critical" active={filter === 'critical'} onClick={() => setFilter('critical')} color="bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-400" />
              <FilterBtn label="High" active={filter === 'high'} onClick={() => setFilter('high')} color="bg-orange-100 text-orange-700 dark:bg-orange-900/50 dark:text-orange-400" />
              <FilterBtn label="Medium" active={filter === 'medium'} onClick={() => setFilter('medium')} color="bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400" />
              <FilterBtn label="Low" active={filter === 'low'} onClick={() => setFilter('low')} color="bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400" />
            </div>
          </div>
        </div>
            ...
            {filteredStates.map(([state, data]) => (
              ...
              >
                ...
                <button 
                  ...
                  className="w-full py-3 mt-auto bg-white dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-slate-700 text-blue-600 dark:text-blue-400 font-bold rounded-xl transition-colors border border-slate-200 dark:border-slate-700 flex items-center justify-center gap-2 shadow-sm"
                >
...
const FilterBtn = ({ label, active, onClick, color }: any) => (
  <button 
    onClick={onClick}
    className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${active ? 'ring-2 ring-offset-2 ring-blue-500 dark:ring-offset-slate-900 ' + color : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
  >
    {label}
  </button>
);
